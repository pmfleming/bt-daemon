...
Outgoing object-push transfers use typed `org.bluez.obex` D-Bus calls. The daemon validates canonical regular files, creates/removes OPP sessions, exposes only opaque request/device IDs plus sanitized basenames, streams byte progress, and supports cancellation. Incoming authorization remains staged.
