...
For connected devices exposing Google's documented Fast Pair Message Stream UUID, an optional backend provider opens the authenticated RFCOMM profile through BlueZ and consumes Device Information battery updates. Verified left, right, and case readings replace BlueZ's aggregate value while the stream is live; unknown components are omitted and the aggregate remains the fallback. BlueZ resolves the device's RFCOMM channel through SDP, so no model or channel is hardcoded. The provider does not implement Fast Pair pairing, account-key exchange, or advertisement tracking.
