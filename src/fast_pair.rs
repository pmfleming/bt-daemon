...
mod keys {
    ...
    impl AccountKeyStore {
        ...
        fn load(path: Option<PathBuf>) -> Result<Self> {
                ...
                .flatten()
            {
                ...
                for (device, encoded) in file.account_keys {
                    ...
                    if key[0] != 0x04 {
                        bail!("Fast Pair account key for {device} has an invalid type byte");
                    }
        ...
        pub(super) fn insert(&self, device_key: String, key: [u8; 16]) -> Result<()> {
            if key[0] != 0x04 {
                bail!("Fast Pair account key has an invalid type byte");
            }
            let mut keys = self
                .keys
                .lock()
    ...
    mod tests {
        ...
        fn keys_survive_a_secure_store_reload() {
            let directory =
                std::env::temp_dir().join(format!("bt-fast-pair-{}", uuid::Uuid::new_v4()));
